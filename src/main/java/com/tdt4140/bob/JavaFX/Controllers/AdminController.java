...
public class AdminController extends Controller {
	...
	/**
	 * A function to change the scene to "user access"-view.
	 * 
	 * @author KimRobin
	 */
	 ...
	 * <p>
	 * Commits a new keyword to the database (curriculum) and confirms/declines
	 * input with user feedback using the "inputControlIsValid()" method.
	 *
	 * @author lajohnso
	 */
	...
	public void onLoad() {
		...
		try {
			...
			// Adds content (TableView) to each tab generated. A TableView for
			// lecturers to see most asked questions.
			while (!code.isEmpty()) {
	...
	public void updateCurriculumView() {
		ResultSet rs = null;
		ResultSet rs3 = null;
		try {
			rs = SettingsHandler.getUserSubjects(app.getDatabaseHandler());
		} catch (SQLException e) {
			e.printStackTrace();
		}
